...
def generar_estado_inicial():
    ...
    posiciones_posibles = [(fila, col) for fila in range(N) for col in range(N)]
    
    # Selecciona posiciones aleatorias del tablero
    posiciones_seleccionadas = random.sample(posiciones_posibles, N)
...
def verificar_conflictos(tablero):
    ...
    for d in range(-(N-1), N):
        diagonal_principal = [tablero[i][i + d] for i in range(N) if 0 <= i + d < N]
        if diagonal_principal.count('O') > 1:
            conflictos += diagonal_principal.count('O') - 1

        diagonal_secundaria = [tablero[i][(N-1) - (i + d)] for i in range(N) if 0 <= (N-1) - (i + d) < N]
        if diagonal_secundaria.count('O') > 1:
            conflictos += diagonal_secundaria.count('O') - 1
...
def generar_vecinos(tablero):
    ...
    for _ in range(50):  # Generamos los vecinos
...
solucion, conflictos, tiempo, movimientos, posiciones = busqueda_tabu()
